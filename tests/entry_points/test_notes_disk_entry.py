...
def test_notes_disk_entry_script_smoke_test():
    cmd = ["python3", "-m", "entry_points.notes_disk_entry"]
    result = subprocess.run(cmd, capture_output=True, check=False)
    assert result.returncode == 0


def test_notes_disk_entry_main_smoke_test():
    args = []
    result = main(args)
    assert result == os.EX_OK


def test_notes_disk_entry_main_show(mocker):
...
def test_notes_disk_entry_main_file_save(tmp_path: str):
